    ...
    <div class="mx-2 md:mx-4 lg:mx-6 relative overflow-hidden rounded-3xl min-h-[360px] md:min-h-[440px]">
        ...
        <div class="absolute bottom-0 left-0 right-0 flex flex-col px-8 pb-12 md:px-12">
            <div class="max-w-[720px] flex flex-col gap-4 md:gap-5">
                @if($featuredNews)
                    <span class="w-fit font-mono text-msp-gold text-xs tracking-widest uppercase">Featured Story</span>
                    ...
                    <a href="{{ route('news.show', $featuredNews->slug) }}"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-msp-gold text-msp-navy font-bold text-sm rounded-xl w-fit transition-all duration-300 hover:brightness-110 hover:shadow-lg">
                        Baca Selengkapnya
                        <i class="fas fa-arrow-right"></i>
                    </a>
                @else
                    <span class="w-fit font-mono text-msp-gold text-xs tracking-widest uppercase">Berita & Artikel</span>
    ...
    <div class="max-w-[1280px] mx-auto px-6">
        <div class="flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex flex-col gap-3 md:max-w-[520px]">
                <span class="font-mono text-msp-gold text-xs tracking-widest uppercase">Newsletter</span>
            ...
            <div class="w-full md:w-auto shrink-0" x-data="{ status: '', msg: '', loading: false }">
                ...
                <form method="POST" action="{{ route('news.subscribe') }}"
                    ...
                    <input id="newsletter-email" type="email" name="email" placeholder="Masukkan email Anda" required
                        class="px-5 py-3 bg-white rounded-xl text-msp-navy text-sm placeholder-msp-gray outline-none focus:ring-2 focus:ring-msp-gold transition-all w-full sm:w-80">
                    <button type="submit" :disabled="loading"
                        class="px-6 py-3 bg-msp-gold text-msp-navy font-bold text-sm rounded-xl whitespace-nowrap transition-all duration-300 hover:brightness-110 hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
