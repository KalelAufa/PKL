...
<div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
    <div class="flex flex-col gap-6 order-2 md:order-1">
        ...
        <h2 class="font-space text-msp-navy text-2xl md:text-3xl lg:text-[38px] leading-snug font-bold">
            Pengendalian Hama Profesional & Tersertifikasi
        </h2>
    ...
    <div class="relative order-1 md:order-2" style="aspect-ratio: 4/3; min-height: 280px;">
        <div class="absolute top-0 left-0 w-[78%] h-[78%] rounded-2xl overflow-hidden shadow-md">
            <img src="{{ $service->gallery_image_1 ? asset('images/' . $service->gallery_image_1) : asset('images/pest-1.jpg') }}"
        ...
        <div class="absolute bottom-0 right-0 w-[55%] h-[55%] rounded-2xl overflow-hidden shadow-lg border-[6px] border-white" style="transform: rotate(-3deg)">
            <img src="{{ $service->gallery_image_2 ? asset('images/' . $service->gallery_image_2) : asset('images/pest-2.jpg') }}"
                 alt="{{ $service->title }}" loading="lazy" class="w-full h-full object-cover">
        </div>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="flex flex-col gap-4 p-7 bg-msp-bg rounded-2xl border border-msp-border">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style="background-color: rgba(11,33,69,0.06);">
                <i class="fas fa-shield-alt text-msp-navy text-sm"></i>
            </div>
            ...
            @if(!empty($feat->bullets))
            <ul class="flex flex-col gap-2">
                @foreach($feat->bullets as $bullet)
                <li class="flex items-start gap-2.5 text-sm text-msp-gray leading-5">
                    <span class="w-4 h-4 rounded-full shrink-0 mt-0.5 flex items-center justify-center" style="background-color: rgba(242,167,27,0.15);">
                        <i class="fas fa-check text-[8px]" style="color: #F2A71B;"></i>
                    </span>
                    {{ $bullet }}
                </li>
                @endforeach
...
{{-- Keunggulan --}}
@php $coreFeatures = $features->where('group', '!=', 'layanan_spesifik')->where('group', null)->values(); @endphp
...
<div>
    <div class="mb-10">
        ...
        <h2 class="font-space text-msp-navy text-2xl md:text-3xl font-bold leading-snug">Metodologi Kami</h2>
    </div>
...
@php
$defaultSteps = [
    ['title' => 'Inspeksi Lokasi', 'desc' => 'Survei menyeluruh untuk identifikasi jenis hama dan tingkat infestasi.'],
    ['title' => 'Perencanaan Solusi', 'desc' => 'Rancangan program pengendalian yang tepat sasaran sesuai kondisi lapangan.'],
    ['title' => 'Eksekusi Treatment', 'desc' => 'Penerapan oleh teknisi bersertifikat dengan bahan aman berstandar tinggi.'],
    ['title' => 'Monitoring Berkala', 'desc' => 'Kunjungan tindak lanjut untuk memastikan efektivitas dan mencegah hama kembali.'],
];
@endphp
<div>
    <div class="mb-10">
...
<div class="flex items-start gap-5 p-7 bg-msp-bg rounded-2xl border border-msp-border">
    ...
    <div class="flex flex-col gap-1.5">
        <h3 class="font-space text-msp-navy text-lg font-bold leading-snug">Brosur Layanan Pest Control</h3>
        <p class="text-msp-gray text-sm leading-6">Detail metodologi, bahan yang digunakan, dan studi kasus pelayanan kami.</p>
